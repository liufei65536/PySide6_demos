...
def init_widget(w, name):
    """Init a widget for the gallery, give it a tooltip showing the
       class name"""
    w.setObjectName(name)       # 设置唯一标识符，  可通过findChild(WidgetClass, ObjName) 获取子控件
    w.setToolTip(class_name(w)) # 提示信息 （鼠标移动到上面的提示）
...
class WidgetGallery(QDialog):
    ...
    def __init__(self):
        ...
        # fix 这里需要加上&，SetBuddy才有用
        style_label = QLabel("&Style:")  # Buddy用法 1.QLabel使用&， &后面的第一个字母s+alt 作为快捷键，可以跳转到setBuddy设置的组件
        init_widget(style_label, "style_label")
        style_label.setBuddy(self._style_combobox) # 2.为label设置Buddy
